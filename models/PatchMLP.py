...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Embedding（默认按 seq_len 自适应 patch；也可 configs.emb_patch_lens 手动指定 4 个尺度）
        emb_pl = getattr(configs, 'emb_patch_lens', None)
        if not (isinstance(emb_pl, (list, tuple)) and len(emb_pl) == 4):
            emb_pl = None
        self.emb = Emb(configs.seq_len, configs.d_model, patch_len=emb_pl)
