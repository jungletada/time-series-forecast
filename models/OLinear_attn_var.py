...
from layers.SelfAttention_Family import (AttentionLayer, EnhancedAttention, VanillaAttention, ProbAttention,
                                         ReformerLayer, FlowAttention, FlashAttention, LinearAttention)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.attn_type.casefold() == 'Enhanced'.casefold():
            self.encoder = Encoder_ori(
                [
                    EncoderLayer(
                        AttentionLayer(
                            EnhancedAttention(False, configs.factor, attention_dropout=configs.dropout,
                                              ...
                                              save_folder=f'./attn_results/{configs.plot_mat_label}_{self.seq_len}'
                                                          f'_{self.pred_len}_011_last_layer'),
        ...
        elif self.attn_type.casefold() == 'Vanilla'.casefold() or self.attn_type.casefold() == 'Transformer'.casefold():
            self.encoder = Encoder_ori(
                [
                    EncoderLayer(
                        AttentionLayer(
                            VanillaAttention(False, configs.factor, attention_dropout=configs.dropout,
        ...
        elif self.attn_type.casefold() == 'Informer'.casefold():
            self.encoder = Encoder_ori(
                [
                    EncoderLayer(
                        AttentionLayer(
        ...
        elif self.attn_type.casefold() == 'Reformer'.casefold():
            self.encoder = Encoder_ori(
                [
                    EncoderLayer(
                        AttentionLayer(
                            ReformerLayer(attention=None, d_model=configs.d_model, n_heads=configs.n_heads),
                            configs.d_model, configs.n_heads),
                        configs.d_model,
                        configs.d_ff,
                        dropout=configs.dropout,
        ...
        elif self.attn_type.casefold() == 'Flowformer'.casefold():
            self.encoder = Encoder_ori(
                [
                    EncoderLayer(
                        AttentionLayer(
                            FlowAttention(attention_dropout=configs.dropout),
                            configs.d_model, configs.n_heads),
                        configs.d_model,
                        configs.d_ff,
                        dropout=configs.dropout,
        ...
        elif self.attn_type.casefold() == 'Flashformer'.casefold():
            self.encoder = Encoder_ori(
                [
                    EncoderLayer(
                        AttentionLayer(
                            FlashAttention(mask_flag=False, attention_dropout=configs.dropout),
                            configs.d_model, configs.n_heads),
                        configs.d_model,
                        configs.d_ff,
                        dropout=configs.dropout,
        ...
        elif self.attn_type.casefold() == 'Flatten'.casefold():
            self.encoder = Encoder_ori(
                [
                    EncoderLayer(
                        AttentionLayer(
                            LinearAttention(mask_flag=False, attention_dropout=configs.dropout, output_attention=False,
                                            mapping_fun='x_3', token_num=configs.enc_in),
                            configs.d_model, configs.n_heads),
                        configs.d_model,
                        configs.d_ff,
                        dropout=configs.dropout,
        ...
        elif self.attn_type.casefold() == 'LinAttn'.casefold():
            self.encoder = Encoder_ori(
                [
                    EncoderLayer(
                        AttentionLayer(
                            LinearAttention(mask_flag=False, attention_dropout=configs.dropout, output_attention=False,
                                            mapping_fun='softmax_learn', token_num=configs.enc_in),
                            configs.d_model, configs.n_heads),
                        configs.d_model,
                        configs.d_ff,
                        dropout=configs.dropout,
        ...
        elif self.attn_type.casefold() == 'Mamba'.casefold():
