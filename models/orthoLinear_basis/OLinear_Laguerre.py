...
class LaguerreTransform(torch.nn.Module):
    def __init__(self, deg=5, T=96, device='cuda:0'):
        super(LaguerreTransform, self).__init__()
        self.deg = deg
        self.T = T
        self.device = device

        tvals = np.linspace(0, 5, T)
        laguerre_polys = np.array([La.basis(i)(tvals) for i in range(self.deg)])
        # tvals = torch.from_numpy(tvals).to(device)
        self.laguerre_polys = torch.from_numpy(laguerre_polys).float().to(device)  # shape: [degree, T]

        weights = np.exp(-tvals)
    ...
    def forward_transform(self, vector):
        ...
        *batch_dims, T = vector.shape
        ...
        weighted_data = data * self.weights[:, None]

        coeffs = torch.mm(self.laguerre_polys, weighted_data) / T
    ...
    def inverse_transform(self, coeffs):
        ...
        *batch_dims, N = coeffs.shape
        ...
        vector = torch.matmul(reshaped_coeffs, self.laguerre_polys)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # chebyshev transform
        self.Laguerre_degree = self.seq_len
        self.transform = LaguerreTransform(self.Laguerre_degree, self.seq_len)
        ...
        self.ortho_trans = nn.Sequential(
            nn.Linear(self.Laguerre_degree * self.embed_size, self.d_model),
            self.encoder,
            nn.Linear(self.d_model, self.Laguerre_degree * self.embed_size)
        )
    ...
    def Fre_Trans(self, x):
        ...
        x_trans = self.ortho_trans(x_trans.flatten(-2)).reshape(B, N, D, self.Laguerre_degree)
