...
class DifferentiableWaveletTransform(torch.nn.Module):
    def __init__(self, wavelet='db1', device='cuda:0'):
    ...
    def forward_transform(self, vector):
        ...
        cA = F.conv1d(vector, dec_lo, stride=2, padding=0, groups=D)
        cD = F.conv1d(vector, dec_hi, stride=2, padding=0, groups=D)

        # Restore the batch dimensions
        cA = cA.reshape(B, N, D, -1)
        cD = cD.reshape(B, N, D, -1)
    ...
    def inverse_transform(self, cA, cD):
        ...
        up_cA = F.conv_transpose1d(cA, rec_lo, stride=2, groups=D)
        up_cD = F.conv_transpose1d(cD, rec_hi, stride=2, groups=D)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.transform = DifferentiableWaveletTransform()
